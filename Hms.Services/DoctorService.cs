...
namespace Hms.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public PagedResult<TimingViewModel> GetAll(
            int pageNumber,
            int pageSize)
        {
            ...
            List<TimingViewModel> vmList =
                new List<TimingViewModel>();
            ...
            var result =
                new PagedResult<TimingViewModel>
                {
                    Data = vmList,
        ...
        {
            var TimingList = _unitOfWork
                .GenericRepository<Timing>()
                .GetAll()
                .ToList();

            var vmList =
                ConvertModelToViewModelList(TimingList);
        ...
        public void UpdateTiming(TimingViewModel timing)
        {
            ...
            ModelById.Doctor = timing.DoctorId;
            ModelById.Status = timing.Status;
            ModelById.Duration = timing.Duration;
            ModelById.MorningShiftStartTime =
                timing.MorningShiftStartTime;

            ModelById.MorningShiftEndTime =
                timing.MorningShiftEndTime;

            ModelById.AfternoonShiftStartTime =
                timing.AfternoonShiftStartTime;

            ModelById.AfternoonShiftEndTime =
                timing.AfternoonShiftEndTime;
        ...
        private List<TimingViewModel>
            ConvertModelToViewModelList(
                List<Timing> modelList)
        {
            return modelList
